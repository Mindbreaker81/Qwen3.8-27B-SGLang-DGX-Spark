...
| `orcarouter/…-Uncensored-NVFP4` (compressed-tensors) | 46.47 → 53.05 tok/s (**+14.1%**) | 24.06 → 25.58 tok/s (**+6.3%**) | official image faster |

The image switch is free on the modelopt export and a real gain on a compressed-tensors one. Unproven but likely: the official image carries 69 upstream commits our `c14312a66` build lacked, the newest being a compressed-tensors scale-loading fix ([#35455](https://github.com/sgl-project/sglang/pull/35455)). Both checkpoints have a dense `lm_head`, so the selector path is not the difference.
...
| `orcarouter/…-Uncensored-NVFP4` | 24.19 → 53.12 tok/s (**2.20×**) | 17.43 → 25.48 tok/s (**1.46×**) |

DFlash2 beats MTP on both probes and both checkpoints, by more than the ~1.5× reported independently in issue #6. MTP was extremely stable (±0.1% across its two boots); DFlash2 carried the session's drift.

**Open question — the MTP column below is stale.** MTP measured 24.2 code / 18.2 essay on 2026-09-05 against the 34.5 / 24.1 recorded on 2026-08-18. That is ~30%, far outside the drift band, and two things differ: the day, and the checkpoint (August ran the packed-FP4-head export, the default is now the dense BF16-head twin). DFlash2 barely moved across the same change, so the dense head may cost MTP much more than it costs DFlash2 — unverified. Until someone re-runs MTP on `QUANT=nvfp4-fp4`, treat the August MTP and DSpark cells as belonging to the FP4-head era, and do not mix them with the table above.

**Method note (2026-09-05):** two boots of the *same* image measured 6.5% apart on the essay probe, larger than most deltas anyone would want to claim. Interleave sides within one session (`SEQ="A B B A"`); never compare across sessions. Also, the first `ndec.py` pass after a boot is unusable — cold start inflates the 60-token call and collapses the two-call denominator, which produced readings of 89, 183 and even −371 tok/s. `bench/ab-image.sh` now discards one pass per boot; a direct `ndec.py` run still needs one thrown away by hand.
