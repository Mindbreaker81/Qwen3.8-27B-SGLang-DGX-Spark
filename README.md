...
| `bench/ab-image.sh` | One-session A/B of the DFlash2 **serving image**: main's self-built image (`IMAGE=` override) vs the official pinned one, same flags, same checkpoint, same draft. Boots each side, verifies the engine from `.sglang.log`, runs `ndec.py` n times, and prints medians plus the delta. `SEQ="A B B A"` cancels linear drift; `DRY_RUN=1` prints the plan. Artifacts land in `bench/_ab-image-<stamp>/` (untracked). |
